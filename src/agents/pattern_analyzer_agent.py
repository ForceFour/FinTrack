    ...
    def process(self, input_data: List[ClassifiedTransaction]) -> Dict[str, Any]:
        ...
        for spike in spikes:
            pattern_insights.append({
                ...
                'confidence': min(1.0, spike['deviation'] / 4)
            })
        ...
        for season in seasonal:
            if season['is_significant']:
                pattern_insights.append({
                    ...
                    'confidence': min(1.0, season['percentage_of_total'] / 50)
                })
        ...
        for category, data in category_trends.items():
            if abs(data['trend']) > 20:
                pattern_insights.append({
                    ...
                    'confidence': min(1.0, abs(data['trend']) / 100)
                })
        
        # Extract top findings
        key_findings = []
        sorted_insights = sorted(pattern_insights, key=lambda x: x['confidence'], reverse=True)
